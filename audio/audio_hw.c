...
#define CARD_PANDABOARD_DEFAULT CARD_OMAP4_ABE
...
struct route_setting defaults[] = {
    /* general */
    {
        .ctl_name = "DL1 Media Playback Volume",
        .intval = 118,
    },
    {
        .ctl_name = "DL1 PDM Switch",
        .intval = 1,
    },
    {
        .ctl_name = "Sidetone Mixer Playback",
        .intval = 1,
    },
    {
        .ctl_name = "DL1 Mixer Multimedia",
        .intval = 1,
    },
    {
    ...
    {
        .ctl_name = "Headset Left Playback",
        .strval = "HS DAC",
    },
    {
        .ctl_name = "Handsfree Right Playback",
        .strval = "HF DAC",
    }
    ,
    {
...
struct pandaboard_audio_device {
    ...
    int in_call;
    float voice_volume;
    struct pandaboard_stream_in *active_input;
    struct pandaboard_stream_out *active_output;
    bool mic_mute;
    int tty_mode;
...
struct pandaboard_stream_out {
    ...
    int standby;
    struct echo_reference_itfe *echo_reference;
    struct pandaboard_audio_device *dev;
    int write_threshold;
    bool low_power;
};
...
static int do_input_standby(struct pandaboard_stream_in *in);
static int do_output_standby(struct pandaboard_stream_out *out);
...
static void set_eq_filter(struct pandaboard_audio_device *adev)
{
...
static void set_input_volumes(struct pandaboard_audio_device *adev, int main_mic_on,
                              int headset_mic_on, int sub_mic_on)
{
    return;
}

static void set_output_volumes(struct pandaboard_audio_device *adev, bool tty_volume)
{
    return;
}

static void force_all_standby(struct pandaboard_audio_device *adev)
{
    struct pandaboard_stream_in *in;
    struct pandaboard_stream_out *out;
...
static void select_mode(struct pandaboard_audio_device *adev)
{
    return ;
}
...
static int start_output_stream(struct pandaboard_stream_out *out)
{
    struct pandaboard_audio_device *adev = out->dev;
    unsigned int card = CARD_PANDABOARD_DEFAULT;
...
static void add_echo_reference(struct pandaboard_stream_out *out,
...
static void remove_echo_reference(struct pandaboard_stream_out *out,
                                  struct echo_reference_itfe *reference)
{
    pthread_mutex_lock(&out->lock);
...
static void put_echo_reference(struct pandaboard_audio_device *adev,
                          struct echo_reference_itfe *reference)
{
    if (adev->echo_reference != NULL &&
...
static struct echo_reference_itfe *get_echo_reference(struct pandaboard_audio_device *adev,
...
static int get_playback_delay(struct pandaboard_stream_out *out,
                       size_t frames,
...
{
    struct pandaboard_stream_out *out = (struct pandaboard_stream_out *)stream;
...
static int start_input_stream(struct pandaboard_stream_in *in)
{
    int ret = 0;
    struct pandaboard_audio_device *adev = in->dev;
...
{
    struct pandaboard_stream_in *in = (struct pandaboard_stream_in *)stream;
...
static void get_capture_delay(struct pandaboard_stream_in *in,
                       size_t frames,
                       struct echo_reference_buffer *buffer)
{
...
static int32_t update_echo_reference(struct pandaboard_stream_in *in, size_t frames)
{
    struct echo_reference_buffer b;
    b.delay_ns = 0;
...
static void push_echo_reference(struct pandaboard_stream_in *in, size_t frames)
{
                                   ...
                                   struct resampler_buffer* buffer)
{
    ...
    in = (struct pandaboard_stream_in *)((char *)buffer_provider -
                                   offsetof(struct pandaboard_stream_in, buf_provider));
...
static ssize_t read_frames(struct pandaboard_stream_in *in, void *buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
static ssize_t process_frames(struct pandaboard_stream_in *in, void* buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
{
    struct pandaboard_audio_device *ladev = (struct pandaboard_audio_device *)dev;
    struct pandaboard_stream_out *out;
    int ret;

    out = (struct pandaboard_stream_out *)calloc(1, sizeof(struct pandaboard_stream_out));
    if (!out)
        return -ENOMEM;
...
{
    struct pandaboard_audio_device *adev = (struct pandaboard_audio_device *)dev;
    struct str_parms *parms;
    char *str;
    char value[32];
    int ret;
                                  ...
                                  struct audio_stream_in **stream_in)
{
    ...
    in = (struct pandaboard_stream_in *)calloc(1, sizeof(struct pandaboard_stream_in));
    if (!in)
        return -ENOMEM;
...
{
    struct pandaboard_audio_device *adev = (struct pandaboard_audio_device *)device;
